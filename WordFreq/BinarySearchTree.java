...
public class BinarySearchTree<T> implements BSTInterface<T> {
	...
	/*
	 * public boolean add (T element) // Adds element to this BST. The tree retains
	 * its BST property. { BSTNode<T> newNode = new BSTNode<T>(element); BSTNode<T>
	 * prev = null, curr = null;
	 * 
	 * if (root == null) root = newNode; else { curr = root; while (curr != null) {
	 * if (comp.compare(element, curr.getInfo()) <= 0) { prev = curr; curr =
	 * curr.getLeft(); } else { prev = curr; curr = curr.getRight(); } } if
	 * (comp.compare(element, prev.getInfo()) <= 0) prev.setLeft(newNode); else
	 * prev.setLeft(newNode); } return true; }
	 */
